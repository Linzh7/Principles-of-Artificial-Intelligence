...
def main():
    ...
    while True:
        ...
        if not result:
            print("from city: {} to city {} search failure".format(
                src_city, dst_city))
        else:
            print("from city: {} to city {} search success".format(
                src_city, dst_city))
            path = []
            while True:
...
def breadth_first_search(src_state, dst_state):
    ...
    while True:
        ...
        if node.parent is not None:
            print("deal node:state:{}\tparent state:{}\tpath cost:{}".format
                  (node.state, node.parent.state, node.path_cost))
        else:
            rint("deal node:state:{}\tparent state:{}\tpath cost:{}".format
                  (node.state, None, node.path_cost))
        '''
        # 遍历
        for i in range(len(_city_info)):
            ...
            # print('\tchild node:state:{} path cost:{}'.format(
                child.state, child.path_cost))
...
def uniform_cost_search(src_state, dst_state):
    ...
    while True:
        ...
        if node.parent is not None:
            print('deal node:state:%s\tparent state:%s\tpath cost:%d' %
                  (node.state, node.parent.state, node.path_cost))
        else:
            print('deal node:state:%s\tparent state:%s\tpath cost:%d' %
                  (node.state, None, node.path_cost))
...
def frontier_priority_replace_by_priority(node):
    global _frontier_priorit
    size=len(_frontier_priority)
    for i in range(size):
        if _frontier_priority[i].state == node.state and _frontier_priority[i].path_cost > node.path_cost:
            # print("\t\t replace state:{} old cost:{} new cost:{}".format(node.state, _frontier_priority[i].path_cost, node.path_cost))
